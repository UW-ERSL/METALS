...
from ExamplesTempDependentMaterials import MMTOTempDependentExamples, getMMTOTempDependentProblem
from materialEncoder import MaterialEncoder # type: ignore[reportMissingImports]
from SensitivitiesTempDependentMaterials import (
...
from PyTOImports import (deflation,linear_solvers,hex_structural_fea, hex_element_stiffness,# type: ignore
                         ...
                         increment_SIMP_STRUCTURAL_PENALTY,increment_SIMP_THERMAL_PENALTY,) # type: ignore

from InterpolationFunctions import bezierInterpolation # type: ignore[reportMissingImports]
...
def run_topopt_temperature_dependent(
    to_problem,
    ...
    def MMTO_optimization_function(zeta):
        nonlocal mmaIterations, obj0, gamma, zRealTorch, last_Temp_nodes, Temp_elem
        zeta = np.asarray(zeta).flatten()
        print("-------------- mmaIterations", mmaIterations, "-----------------")
        ...
        if turnOnThermal:
            if turnOnNonlinearThermal:
                ...
                for picard_iter in range(max_picard_iter):
                    ...
                    print(f"Max Picard mmaIterationss reached with norm: {norm_diff:.6e}")
...
if __name__ == "__main__":
    ...
    run_topopt_temperature_dependent(
