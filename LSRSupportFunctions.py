...
def optimizationFunction_structural(
    x, fe_solver, to_params, vae_info, num_elems, num_design_var, H, Hs, KE, materialEncoder, shared_vars, gamma=100, debug=False, apply_filter_to_materials=True
):
...
def optimizationFunction_tempdependent(
    x, fe_solver_structural, fe_solver_thermal, to_params, materialEncoder, num_elems, num_design_var, H, Hs, KE, shared_vars, gamma=100, debug=False, apply_filter_to_materials=True
):
    if 'J0' not in shared_vars or shared_vars['J0'] is None:
        shared_vars['J0'] = None
    ...
    zDesign = zD.view(2, -1).T
    decoded = materialEncoder.vaeNet.decoder(zDesign)
    Ea, Eb, Ec, Ed, massDensity, thermalConductivity = materialEncoder.getMaterialProperties_tempdependent(decoded)
    # --- THERMAL ANALYSIS ---
    thermalConductivity_elem = thermalConductivity
    ...
    md_elem = massDensity_c.detach().cpu().numpy()
...
def optimizationFunction_structuralcost(
    x, fe_solver, to_params, num_elems, num_design_var, H, Hs, KE, materialEncoder, shared_vars, gamma=100, debug=False, apply_filter_to_materials=True
):
    if 'J0' not in shared_vars or shared_vars['J0'] is None:
        shared_vars['J0'] = None
    ...
    Z_data = materialEncoder.training_latents.to(zDesign.device)  # shape (N_train, latentDim)
    p_softmin = -6
    d_ij = torch.cdist(zDesign, Z_data, p=2) + 1e-6  
    soft_i = torch.sum(d_ij ** p_softmin, dim=1).pow(1.0/p_softmin)
    penalty = gamma * torch.sum(soft_i)/num_elems
    ...
    xTensor.grad = None
    penalty.backward(retain_graph=True)
    dpen = xTensor.grad[num_elems:].detach().numpy().reshape(-1, 2) < 0  # shape (num_elems, latentDim)
    grad_obj[num_elems:,0] += dpen.flatten()      
    return obj, grad_obj, cons, grad_cons
...
def optimizationFunction_structuralyield(
    ...
    if (gamma > 0):  # --- Latent space penalization ---
        Z_data = materialEncoder.training_latents.to(zDesign.device)  # shape (N_train, latentDim)
        p_softmin = -6
        d_ij = torch.cdist(zDesign, Z_data, p=2) + 1e-6  # shape (num_elems, N_train)
        soft_i = torch.sum(d_ij ** p_softmin, dim=1).pow(1.0 / p_softmin)
        penalty = gamma * torch.sum(soft_i) / num_elems
