...
def compute_mmto_objective_and_gradient(to_params, uvw, Temp, zeta, fe_solver_structural, KETemplate, matEncoder):
    ...
    if objectiveType == TO_QOI.COMPLIANCE:
        ETensor = matEncoder.getMaterialPropertyAtTemperatureTorch("E", zetaTensor[num_elems:].view(2, -1).T, Temp)
...
def compute_mmto_constraint_and_gradient(to_params, uvw,T, zeta, fe_solver_structural, KETemplate, matEncoder):
    ...
    for m in range(nConstraints):
        ...
        if constraintType == TO_QOI.COMPLIANCE:
            ...
            ETensor = matEncoder.getMaterialPropertyAtTemperatureTorch("E", zetaTensor[num_elems:].view(2, -1).T, T)
            YTensor = matEncoder.getMaterialPropertyAtTemperatureTorch("Y", zetaTensor[num_elems:].view(2, -1).T, T)
            EDesign= ETensor.detach().numpy()
            ...
            grad_inv_sf_z = np.zeros(2*num_elems)
            ...
            d_sigma_dz_0 = d_sigma_vm_dE * dE_dz[:,0]
            d_sigma_dz_1 = d_sigma_vm_dE * dE_dz[:,1]
            d_inv_sf_dz_0 = (d_sigma_dz_0 * Y - dY_dz[:,0]*sigma_vm) / (Y ** 2)
            d_inv_sf_dz_1 = (d_sigma_dz_1 * Y - dY_dz[:,1]*sigma_vm) / (Y ** 2)
            grad_inv_sf_z[0:num_elems] = (pNormExponent * (inv_sf_elem ** (pNormExponent - 1))) * d_inv_sf_dz_0
            grad_inv_sf_z[num_elems:] = (pNormExponent * (inv_sf_elem ** (pNormExponent - 1))) * d_inv_sf_dz_1
