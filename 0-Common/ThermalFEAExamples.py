...
class ThermalFEAExamples(enum.Enum):
...
def createBliskSectionThermalProblem_TempBC(mesh=None, nDOFDesired: int = 10000, 
    ...
    if mesh is None:
        from StructuralFEAExamples import createBliskSectionProblem
...
def createLBracketThermalProblem(mesh=None, nDOFDesired: int = 10000, 
                                 temp_at_load=250, temp_at_wall=50, thermal_conductivity=1.0):
    """
    ...
    # If mesh is not provided, create it using the structural example
    if mesh is None:
        from StructuralFEAExamples import createLBracketProblem
...
# Example usage:
# mesh, mat_prop, bc = getThermalFEAProblem(ThermalFEAExamples.EdgeCantilever, mesh=structural_mesh)